...
export const Auth2: React.FC<Auth2Props> = ({
  ...
  const handleOAuthNotice = (providerName: string) => {
    setErrorMsg(
      language === 'ru'
        ? `Вход через ${providerName} находится на стадии модерации платформы. Войдите через Google или Email.`
        : `${providerName} sign-in is completing platform verification. Please use Google or Email.`
    );
  };
  ...
  const handleGoogleAuth = async () => {
    ...
    try {
      ...
      if (
        ...
        setErrorMsg(language === 'ru' ? 'Окно авторизации было закрыто' : 'Sign-in popup closed by user');
      } else if (err?.code === 'auth/popup-blocked') {
        setErrorMsg(language === 'ru' ? 'Всплывающее окно заблокировано браузером. Разрешите всплывающие окна в настройках.' : 'Popup blocked by browser. Please allow popups in settings.');
      } else {
        ...
        <div className="max-w-md w-full mx-auto space-y-6">
          ...
          {isUnauthorizedDomain ? (
            <motion.div
              ...
              className="p-3.5 sm:p-4 rounded-2xl bg-amber-950/40 border border-amber-500/50 text-amber-200 text-xs space-y-2.5 shadow-lg shadow-amber-950/30"
            >
              <div className="flex items-start gap-2.5">
                <AlertTriangle className="w-5 h-5 text-amber-400 shrink-0 mt-0.5" />
                <div className="space-y-1">
                  <h4 className="font-bold text-amber-300 text-xs sm:text-sm">
                    {language === 'ru' ? 'Google OAuth ограничен на внешнем домене' : 'Google OAuth restricted on external domain'}
                  </h4>
                  <p className="text-[11px] text-amber-200/90 leading-relaxed">
                    {language === 'ru'
                      ? 'В облачной песочнице Google OAuth требует подтверждения в корпоративном аккаунте. Используйте мгновенный вход или регистрацию по Email/Никнейму ниже — он работает на 100% прямо сейчас!'
                      : 'Google OAuth is restricted by security policy on external domains. Please use the instant Email/Username registration below — it is 100% active and working right now!'}
                  </p>
                </div>
              </div>

              <div className="pt-1 flex items-center justify-between border-t border-amber-500/20 text-[11px]">
                <span className="text-emerald-400 font-medium flex items-center gap-1.5">
                  <Sparkles className="w-3.5 h-3.5 text-emerald-400 shrink-0" />
                  <span>{language === 'ru' ? 'Заполните форму ниже для входа' : 'Fill in the form below to proceed'}</span>
                </span>
                <button
                  type="button"
                  onClick={() => {
                  ...
                  className="px-2.5 py-1 rounded-lg bg-amber-500/20 hover:bg-amber-500/30 text-[10px] font-bold text-amber-300 transition-colors cursor-pointer"
                >
          ...
          <div className="flex items-center gap-3 pt-1">
            ...
            <span className="text-[10px] text-slate-500 font-mono uppercase tracking-widest">
              {language === 'ru' ? 'или войти через сервисы' : 'or continue with'}
            </span>
            <div className="flex-1 border-t border-[#16273C]" />
          </div>
          ...
          <div className="space-y-2.5">
            <button
              ...
              disabled={isLoading}
              className="w-full py-2.5 px-4 rounded-full bg-[#0C1524] hover:bg-[#111F36] border border-[#1A2D48] hover:border-emerald-500/50 text-xs font-semibold text-white flex items-center justify-center gap-3 transition-all shadow-sm cursor-pointer active:scale-98 group"
            >
              ...
              <span>{language === 'ru' ? 'Google' : 'Google'}</span>
            </button>

            <div className="grid grid-cols-2 gap-2.5">
              <button
                type="button"
                onClick={() => handleOAuthNotice('GitHub')}
              ...
              <button
                type="button"
                onClick={() => handleOAuthNotice('Apple ID')}
                className="py-2 px-3 rounded-full bg-[#0C1524] hover:bg-[#101D30] border border-[#1A2D48] hover:border-slate-600 text-xs font-medium text-slate-300 hover:text-white flex items-center justify-center gap-2 transition-all cursor-pointer"
              >
